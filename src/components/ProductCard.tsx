...
export default function ProductCard({ product, onQuickView }: Props) {
  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-square overflow-hidden bg-gray-50 border-b-2 border-[#1a1a2e]">
        ...
        <div className="absolute bottom-0 left-0 right-0 translate-y-full group-hover:translate-y-0 transition-transform duration-200 flex">
          <button
            ...
            className="flex-1 bg-white/95 backdrop-blur-sm border-t-2 border-r border-[#1a1a2e] py-2 text-xs font-black flex items-center justify-center gap-1.5 hover:bg-gray-50 transition-colors"
          >
            <Eye size={12} /> Quick View
          </button>
          <button
            onClick={handleAdd}
            className="flex-1 bg-[#6c47ff] border-t-2 border-[#1a1a2e] py-2 text-xs font-black text-white flex items-center justify-center gap-1.5 hover:bg-[#5a38e0] transition-colors"
          >
      ...
      {/* Body */}
      <div className="p-3">
        <p className="text-[10px] text-gray-400 font-semibold uppercase tracking-wide mb-0.5">{product.seller}</p>
        <h3 className="text-sm font-bold text-gray-900 leading-snug mb-1.5 line-clamp-2">{product.title}</h3>
        ...
        {product.rating && (
          <div className="flex items-center gap-1 mb-2">
            <div className="flex">
              {[1,2,3,4,5].map((i) => (
                <Star key={i} size={10} className={i <= Math.round(product.rating!) ? 'text-amber-400 fill-amber-400' : 'text-gray-200 fill-gray-200'} />
              ))}
            </div>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-baseline gap-1.5">
            <span className="text-base font-black text-[#6c47ff]">${product.price}</span>
            {product.originalPrice && (
              <span className="text-xs text-gray-400 line-through">${product.originalPrice}</span>
            )}
          </div>
          <span className="flex items-center gap-0.5 text-[10px] text-gray-400">
            <MapPin size={9} />{product.distance}mi
          </span>
        </div>
        ...
        {product.stock && product.stock < 15 && (
          <div className="mt-2">
            ...
            <p className="text-[10px] text-orange-500 font-bold mt-0.5">Only {product.stock} left!</p>
          </div>
        )}
      </div>
    </article>
