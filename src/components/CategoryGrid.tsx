      ...
      <div className="grid grid-cols-4 sm:grid-cols-4 md:grid-cols-8 gap-2 sm:gap-3">
        {CATEGORIES.map((cat) => (
          <button
            ...
            className={clsx(
              'flex flex-col items-center gap-1.5 rounded-xl sm:rounded-2xl border-2 overflow-hidden transition-all duration-200 hover:scale-105 active:scale-95',
              active === cat.id
                ? 'border-[#6c47ff] shadow-[3px_3px_0_#6c47ff]'
                : 'border-[#1a1a2e] shadow-[2px_2px_0_#1a1a2e] sm:shadow-[3px_3px_0_#1a1a2e] hover:shadow-[4px_4px_0_#1a1a2e]'
            )}
          >
            <div className="relative w-full aspect-square overflow-hidden">
              <img src={cat.image} alt={cat.name} className="w-full h-full object-cover" loading="lazy" />
              <div className={clsx('absolute inset-0 transition-opacity', active === cat.id ? 'bg-[#6c47ff]/30' : 'bg-black/10')} />
            </div>
            <span className={clsx(
              'text-[9px] sm:text-[10px] font-black pb-1.5 sm:pb-2 px-0.5 text-center leading-tight w-full truncate',
